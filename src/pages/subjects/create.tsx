 ...
 * Create Subject page.
 *
 * Provides a form to create a new subject and associates it with a department.
 * The demo relies on:
 * - `useList("departments")` to populate the dropdown
 * - disabling submission until a valid `departmentId` is selected (prevents 422s)
 */
...
const SubjectsCreate = () => {
  ...
  // Depending on the REST data provider mapping + pagination mode,
  // `useList` may store items either at `query.data.data` or directly in `query.data`.
  const departments = Array.isArray(departmentsQuery.data)
    ? (departmentsQuery.data as Department[])
    : ((departmentsQuery.data as { data?: Department[] } | undefined)?.data ??
        []);
  ...
  // Used to prevent invalid/placeholder selections from being submitted.
